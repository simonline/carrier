...
pub fn build_cli() -> App<'static, 'static> {
    let clap = App::new(crate_name!())
        ...
        .subcommand(
            ...
            .about("change carrier configuration")
            .arg(Arg::with_name("identity").short("-t").long("target").takes_value(true).required(false).help("config a remote device"))
            .arg(Arg::with_name("otp").short("-k").long("otp").takes_value(true).required(false).help("one time password for remote device"))
            .setting(clap::AppSettings::ArgRequiredElseHelp)
            .subcommand(
                SubCommand::with_name("authorize")
                .about("add an authorized peer")
                .arg(Arg::with_name("identity").takes_value(true).required(true))
                .arg(Arg::with_name("resource").takes_value(true).required(true))
            )
            .subcommand(
                SubCommand::with_name("deauthorize")
                .about("remove an authorized peer")
                .arg(Arg::with_name("identity").takes_value(true).required(true))
                .arg(Arg::with_name("resource").takes_value(true).required(true))
            )
            .subcommand(
                ...
                .arg(Arg::with_name("secret").takes_value(true).required(true))
            )
        )
        .subcommand(
